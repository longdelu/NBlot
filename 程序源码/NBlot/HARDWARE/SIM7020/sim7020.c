...
//    memcpy(tmp_buf,g_sim7020_recv_desc.buf, 512); 
    ...
    if (sim7020_handle->sim7020_event & SIM7020_RECV_EVENT) 
    {
        ...
        //如果命令响应结果正确      
        if ((cmd_is_pass == AT_CMD_RESULT_OK) || (cmd_is_pass == AT_CMD_RESULT_CONTINUE)) 
        {            
        ...
        {              
            ...
            if (g_at_cmd.cmd_action & ACTION_ERROR_AND_TRY) { 
                ...
                printf("%s cmd is failed and next\r\n", g_at_cmd.p_atcmd);
        ...
        {
            //理论上不会运行到这里, 运行到这里，表示传输出错，收到的数据都是乱码        
            //延时一段时间，防止还有数据过来，这些数据都是垃圾数据
...
static int8_t at_cmd_result_parse(char* buf)
{
    ...
    if(g_at_cmd.p_expectres == NULL)
    {
        ...
        } else if ((strstr(buf,g_at_cmd.p_atcmd)) || (strstr(buf,p_colon))) {
   ...
   if((target_pos_start = strstr(buf, "+CGREG:")) != NULL)
    ...
    else if((target_pos_start = strstr(buf,"+CSONMI")) != NULL)
    {
        ...
        if(p_colon)
        {
          ...
          g_socket_info[0].data_offest = p_colon - target_pos_start;
        }               
        //得到收到的数据长度
        ...
        {
          pComma++;
          g_socket_info[0].data_offest = pComma - p_colon;
        ...
        {
          p_data_offest++;
          g_socket_info[0].data_offest += p_data_offest- pComma;
        }
...
//    
  ...
  else if(g_sim7020_status.main_status == SIM7020_TCPUDP_RECV)
  {
    ...
    {
      char *data_buf = &g_sim7020_recv_desc.buf[g_socket_info[0].data_len]; 
  ...
  else if(g_sim7020_status.main_status == SIM7020_SOCKET_ERR)
  {
    if(g_sim7020_status.sub_status == SIM7020_SUB_SOCKET_ERR)
    {
      ...
      printf("the socket err, the err code is %d\r\n", g_socket_info[0].socket_errcode); 
 
          
      sim7020_handle->sim7020_cb(sim7020_handle->p_arg,(sim7020_msg_id_t)SIM7020_MSG_TCPUDP_RECV, 2, "F");
