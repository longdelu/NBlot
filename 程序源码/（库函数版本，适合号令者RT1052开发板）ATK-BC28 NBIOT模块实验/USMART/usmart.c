...
{     
    PIT->CHANNEL[3].TFLG|=1<<0;            //清除PIT CH3中断
    PIT->CHANNEL[3].TCTRL&=~(1<<0);        //关闭CH4计数 
    PIT->CHANNEL[3].LDVAL=0XFFFFFFFF;    //设置倒计时最大延时 
    usmart_dev.runtime=0;    
    PIT->CHANNEL[3].TCTRL|=1<<0;        //使能CH4计数 
}
//获得runtime时间
...
{
    if(PIT->CHANNEL[3].TFLG&(1<<0))        //在运行期间,产生了定时器溢出
    {
        usmart_dev.runtime+=572662;        //75M计数频率,32位溢出,时间为57.2662秒
    }
    usmart_dev.runtime+=(0XFFFFFFFF-PIT->CHANNEL[3].CVAL)/7500;
    //重新设置PIT CH3的定时时间
    PIT->CHANNEL[3].TFLG|=1<<0;            //清除PIT CH3中断
    PIT->CHANNEL[3].TCTRL&=~(1<<0);        //关闭CH4计数 
    PIT->CHANNEL[3].LDVAL=7500000;        //设置0.1ms倒计时  
    PIT->CHANNEL[3].TCTRL|=1<<0;        //使能CH4计数 
...
{ 
    if(PIT->CHANNEL[3].TFLG&(1<<0))    //PIT CH4中断
    { 
...
//初始化PIT定时器CH1中断 
//ldval: CH1初始值,0~0xFFFFFFFF
//递减计数,当ldval==0时,产生中断.
//定时时间=ldval/PERCLK_CLK_ROOT
void PIT_CH4_Int_Init(u32 ldval)
{ 
    CCM->CCGR1|=(u32)3<<12;            //使能PIT时钟,来自PERCLK_CLK_ROOT=75Mhz
    PIT->MCR=0<<1;                    //使能PIT定时器
    PIT->MCR|=0<<0;                    //DEBUG模式下继续跑
    PIT->CHANNEL[3].LDVAL=ldval;    //设置倒计时初始值
    PIT->CHANNEL[3].TCTRL=1<<1;        //使能CH4的中断 
    RT1052_NVIC_SetPriority(PIT_IRQn,10,0);//抢占优先级位10，0位子优先级
    EnableIRQ(PIT_IRQn);            //使能PIT_IRQn的中断
    PIT->CHANNEL[3].TCTRL|=1<<0;    //使能CH4计数 
}
#endif
////////////////////////////////////////////////////////////////////////////////////////
...
{
#if USMART_ENTIMX_SCAN==1
    PIT_CH4_Int_Init((u32)sysclk*100000-1);    //100ms中断一次,注意,计数频率必须为10Khz,以和runtime单位(0.1ms)同步.
#endif
...
//如果非ALIENTEK用户,则LPUART_RX_STA和LPUART_RX_BUF[]需要用户自己实现
void usmart_scan(void)
{
    u8 sta,len;  

    if(LPUART_RX_STA&0x8000)//串口接收完成？
    {                       
        len=LPUART_RX_STA&0x3fff;    //得到此次接收到的数据长度
        LPUART_RX_BUF[len]='\0';    //在末尾加入结束符. 
        sta=usmart_dev.cmd_rec(LPUART_RX_BUF);//得到函数各个信息
        if(sta==0)usmart_dev.exe();    //执行函数 
        else 
        {  
            len=usmart_sys_cmd_exe(LPUART_RX_BUF);
