...
u32 LCD_ReadPoint(u16 x,u16 y)
{
    ...
    if(lcdelcdif.pwidth!=0)                            //如果是RGB屏
    {
        return ELCDIF_Read_Point(x,y);
    }
    LCD_SetCursor(x,y);        
...
{                       
    if(lcdelcdif.pwidth!=0)ELCDIF_Switch(1);//开启LCD
...
{
    if(lcdelcdif.pwidth!=0)ELCDIF_Switch(0);//关闭LCD
...
void LCD_DrawPoint(u16 x,u16 y)
{
    if(lcdelcdif.pwidth!=0)//如果是RGB屏
    {
        ELCDIF_Draw_Point(x,y,POINT_COLOR);
    }else
    {
...
void LCD_Display_Dir(u8 dir)
{
    ...
    if(lcdelcdif.pwidth!=0)   //如果是RGB屏
    {
        ELCDIF_Display_Dir(dir);
        lcddev.width=lcdelcdif.width;
        lcddev.height=lcdelcdif.height;
        return;
    }
    if(dir==0)            //竖屏
    {
...
{ 
    ...
    lcddev.id=ELCDIF_PanelID_Read();//检查是否有RGB屏接入
     ...
     }else                            //检查是否有MCU屏接入
    {
        ...
        //针对MCU屏，必须修改0X80500000开始的SDRAM地址空间属性为不可缓存内存，否则无法正常使用MCU屏
        ...
        GPIO_PinInit(GPIO3,2,&lcdbl_config);         //初始化GPIO2_03
        ...
        //初始化DBI接口
        semcclock=CLOCK_GetFreq(kCLOCK_SemcClk);        //获取SEMC时钟，132
...
{  
    ...
    if(lcdelcdif.pwidth!=0)    //如果是RGB屏
    {
        ELCDIF_Color_Fill(sx,sy,ex,ey,color);
    }else
    {
